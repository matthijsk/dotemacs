...
#+BEGIN_SRC emacs-lisp
  (use-package evil
    ...
    (defun my-evil-forward-sentence (&optional arg)
      "Call `forward-sentence' after incrementing point by 1.
  ...
  Info node `(evil)Modes and states'."
    ...
    (evil-define-key 'normal text-mode-map (kbd "M-e") 'my-evil-forward-sentence)
    (evil-define-key 'normal emacs-lisp-mode-map (kbd "M-e") 'my-evil-forward-sentence)
...
#+BEGIN_SRC emacs-lisp
  ...
  The file is created if it does not exist. It is structured as an
  org mode date tree, the difference being the clock is started on
  the day heading instead of a subheading, which would be the case
  if the date tree was created using org-capture. See Info
  node `(org)Using capture' and Info node
