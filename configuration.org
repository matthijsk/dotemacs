...
** Packages and use-package
  To ensure org-mode is properly initialized before loading this file using
  ~org-babel-load-file~ the ~package-archives~ settings and the call to
  ~package-initialize~ are done in =[[file:init.el][init.el]]=.
