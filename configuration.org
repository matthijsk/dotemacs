...
Set full user path (ignoring =$HOME=). Useful on Windows systems where =$HOME=
is not set or cases where a full path is required.
#+begin_src emacs-lisp
  (when (eq system-type 'windows-nt)
    (setq my-user-dir (format "c:/Users/%s" user-login-name)))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  ...
  Sets face to the face `org-todo'."
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org
    ...
    (when (boundp 'my-user-dir)
      (setq org-directory (concat my-user-dir "/org")))
