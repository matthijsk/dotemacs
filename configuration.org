...
Enable global minor mode Editorconfig.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  ;; Fall back if spacemacs-theme is not available.
...
Most of the time I'm interested in the number of lines in a buffer, not the
number of lines on a page (an Emacs concept I never use). So remap count-lines
to count-words, which displays the number of lines in a buffer.
#+begin_src emacs-lisp
  (bind-key [remap count-lines-page] 'count-words)
#+end_src
...
Show character names in ~what-cursor-position~. Very useful for hard to
distinguish characters such as =ZERO WIDTH SPACE=.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  use default modifiers."
    ...
    ;; Custom move frame commands. Emacs eats some keys so the frame can be moved
    ;; using the OS keys.
    (defun my-move-frame (position)
      "Resize current frame to half the screen width and position it at the left or right edge.
  POSITION is a symbol: 'left or 'right."
      (interactive
       (list (intern (completing-read "Position: " '(left right) nil t))))
      (let* ((display-pixel-width (display-pixel-width))
             (display-pixel-height (display-pixel-height))
             (half-width (/ display-pixel-width 2))
             (x (if (eq position 'left) 0 (- display-pixel-width half-width))))
        (set-frame-size (selected-frame) half-width display-pixel-height t)
    ...
    (bind-key "s-<left>" #'my-move-frame-left)
    (bind-key "s-<right>" #'my-move-frame-right)
    (bind-key "s-<up>" #'toggle-frame-maximized)
...
#+begin_src emacs-lisp
  (bind-key "C-c E" #'(lambda () "Insert \"€\" (EURO SIGN) at point." (interactive) (insert-char ?€)))
#+end_src
...
Convenience function to start an interactive shell in a Django project.
#+begin_src emacs-lisp
  (defun my-project-django-shell ()
    "Start an interactive shell in a django project."
    (interactive)
    (let ((default-directory (project-root (project-current t)))
          (python-shell-interpreter-interactive-arg "-i python"))
      (run-python "./manage.py shell" 'project t)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package eshell
    :commands (eshell project-eshell)
...
#+begin_src emacs-lisp
  (use-package calendar
    ...
    (holiday-other-holidays '((holiday-fixed 4 27 "King's Day")
    ...
    (calendar-latitude 52.2)
    (calendar-longitude 5.4)
    (calendar-location-name "Amersfoort")
...
#+begin_src emacs-lisp
  (use-package project
    :config
    (add-to-list 'project-switch-commands '(consult-ripgrep "Consult ripgrep" "r") t)
...
#+end_quote
Set fido-vertical-mode as fallback for when Vertico, Orderless, Consult and
Marginalia (great combo!) are not available.
#+BEGIN_SRC emacs-lisp
  (use-package vertico
...
function completing-read documented in the Elisp manual.
#+end_quote
...
#+BEGIN_SRC emacs-lisp
  (use-package consult
    ...
    :pin melpa-stable
    :after (project evil-leader)
    :commands (my-consult-line consult-ripgrep)
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (org-todo-keywords '((sequence "TODO" "IN PROGRESS" "REVIEW" "|" "DONE" "WONTFIX" )))
    ...
    (org-agenda-files (concat (file-name-as-directory org-directory) "org-agenda-files.org"))
    ...
    ;; Add "p" menu key to quickly insert python source blocks using `org-insert-structure-template'.
    (add-to-list 'org-structure-template-alist '("p" . "src python-ts"))
  ...
  the default value is used."
    ...
    (defcustom my-org-agenda (concat (file-name-as-directory org-directory) "agenda.org")
      "Org file for logging repeating work items, such as stand-ups and other
  repeating meetings."
      :type 'file
      :group 'work)
  ...
  Asks to clock in for the \"Daily stand-up\" work item in file
  `my-org-agenda' Then displays the clocktable, the agenda and
  `my-org-worklog' in a split window with the following layout:
  ...
  "
      (interactive)
      (when (y-or-n-p "Clock in for daily standup?")
        (with-current-buffer (find-file-noselect my-org-agenda)
          (goto-char (point-min))
      ...
      (let ((org-clock-clocktable-default-properties
             ;; Show clock table for past three days when today is Monday.
             (if (= (calendar-day-of-week (calendar-current-date)) 1)
                 '(:maxlevel 3 :scope agenda :fileskip0 t :tstart "<-3d>" :tend "<today>" :step day :narrow 120!)
               '(:maxlevel 3 :scope agenda :fileskip0 t :block yesterday :step day :narrow 120!))))
        (my-org-clock-report))
      (org-agenda-list)
      (other-window 1)
      (split-window-below)
      (switch-to-buffer-other-window (find-file-noselect my-org-worklog))
...
#+begin_src emacs-lisp
  (defcustom my-use-tree-sitter nil
    "Use tree-sitter enhanced modes and grammars."
    :type 'boolean
    :group 'initialization)

  (use-package treesit
    :if my-use-tree-sitter
    :ensure nil
    :config
    (progn (setq treesit-language-source-alist
...
#+begin_src emacs-lisp
  (use-package jinx
    :pin melpa-stable
    :if (and (memq system-type '(gnu/linux darwin)) (package-installed-p 'jinx))
    :hook (org-mode-hook git-commit-mode-hook)
    :bind (("M-$" . jinx-correct)
           ("C-M-$" . jinx-languages)
           :map evil-normal-state-map
           ("z=" . jinx-correct)))
...
#+begin_src emacs-lisp
  (use-package pyvenv
    :commands (my-venv-activate my-venv-deactivate)
...
#+begin_src emacs-lisp
  (use-package sqlformat
    ...
    (sqlformat-args '("--nocolor" "--disable-progress-bar")))
#+end_src
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    :bind (("C-c g" . (lambda () "Call `gptel-send' with prefix ARG enabled." (interactive) (gptel-send '(4)))))
    :custom
    ...
    (let ((base-system-message "Respond concisely. Never apologize. Cite sources with your answers if possible.\n"))
      (gptel-make-preset 'python-django
        :description "Python and Django programming"
        :system (concat base-system-message "You are a Python programmer and a Django expert.
  Replace version numbers in links to docs.djangoproject.com with \"stable\"."))

      (gptel-make-preset 'bash-shell
        :description "Bash shell scripting"
        :system (concat base-system-message "You are a Bash scripting expert.")))

    (defun my-gptel-context-remove-all ()
      "Remove all gptel context without asking for confirmation."
      (interactive)
