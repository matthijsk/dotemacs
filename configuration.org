...
Clean up whitespace when saving certain buffers.
#+begin_src emacs-lisp
  (setq my-whitespace-cleanup-modes '(org-mode markdown-mode python-mode))
...
next to the spacebar. I suspect I won't be using the super key much so don't set
that modifier.
#+begin_src emacs-lisp
